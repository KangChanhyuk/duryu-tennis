import streamlit as st
import pandas as pd
import random
import os
from datetime import datetime
import itertools

# =============================
# 페이지 설정 및 CSS
# =============================
st.set_page_config(layout="wide", page_title="두류 테니스 랭킹 시스템")

st.markdown("""
    <style>
    .main-title { text-align: center; color: #1E88E5; font-size: 3rem; font-weight: bold; margin-bottom: 20px; }
    .team-card { border-radius: 10px; padding: 15px; margin-bottom: 10px; box-shadow: 2px 2px 5px rgba(0,0,0,0.1); text-align: center; }
    .match-bg-1 { background-color: #f1f8e9; border-left: 8px solid #4CAF50; }
    .match-bg-2 { background-color: #e3f2fd; border-left: 8px solid #2196F3; }
    .team-name { font-size: 1.2rem; font-weight: bold; color: #333; text-align: center; }
    .vs-text { font-size: 1.5rem; font-weight: bold; color: #E53935; text-align: center; line-height: 80px; }
    input { text-align: center !important; }
    .stButton>button { width: 100%; border-radius: 5px; }
    th, td { text-align: center !important; border: 1px solid #ddd !important; }
    </style>
    """, unsafe_allow_html=True)

# =============================
# 데이터 파일 및 상태 관리
# =============================
RANK_FILE = "ranking_master.csv"
HISTORY_FILE = "history_master.csv"

# 한울방식 KDK 대진표 데이터 (이미지 기준 학습 데이터)
# A:10, B:11, C:12로 치환하여 정의
HANUL_KDK_DATA = {
    "3": { # 1인 3게임 기준
        4: ["14:23", "13:24", "12:34"],
        8: ["12:34", "56:78", "18:27", "36:45", "14:58", "23:67"],
        12: ["12:34", "56:78", "910:1112", "13:57", "24:68", "911:1012", "48:912", "67:1011", "1112:23"]
    },
    "4": { # 1인 4게임 기준
        5: ["12:34", "13:25", "14:35", "15:24", "23:45"],
        6: ["13:24", "15:46", "23:56", "14:35", "26:34", "16:25"],
        7: ["12:34", "56:17", "23:57", "14:67", "35:24", "16:25", "46:37"],
        8: ["12:34", "56:78", "13:57", "24:68", "15:26", "37:48", "16:38", "25:47"],
        9: ["12:34", "56:78", "19:57", "23:68", "49:38", "15:26", "36:45", "17:89", "24:79"],
        10: ["12:35", "67:810", "23:46", "78:19", "34:57", "89:210", "45:68", "13:910", "56:79", "110:24"],
        11: ["12:35", "67:810", "49:111", "23:68", "45:710", "911:26", "13:711", "48:59", "110:28", "47:611", "39:510"]
    }
}

def init_state():
    defaults = {
        "players": [], "groups": {}, "modes": {}, "game_counts": {},
        "schedule": {}, "scores": {}, "current_date": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "tournament_name": "정기 대회"
    }
    for k, v in defaults.items():
        if k not in st.session_state: st.session_state[k] = v

init_state()

# =============================
# 핵심 로직 함수
# =============================
def load_rank():
    if not os.path.exists(RANK_FILE): return pd.DataFrame(columns=["이름", "현재포인트"])
    df = pd.read_csv(RANK_FILE)
    if "현재포인트" in df.columns:
        df["현재포인트"] = pd.to_numeric(df["현재포인트"], errors='coerce').fillna(0).astype(int)
    return df

def save_rank(df):
    if "현재포인트" in df.columns: df = df.sort_values("현재포인트", ascending=False).reset_index(drop=True)
    df.to_csv(RANK_FILE, index=False)

def load_history():
    if not os.path.exists(HISTORY_FILE): return pd.DataFrame(columns=["날짜", "대회명", "그룹", "방식", "이름", "순위", "승", "득실"])
    return pd.read_csv(HISTORY_FILE)

def team_name(t, p_list=None):
    if isinstance(t, (list, tuple)):
        names = []
        for p in t:
            prefix = f"({p_list.index(p)+1})" if p_list and p in p_list else ""
            names.append(f"{prefix}{p}")
        return " & ".join(names)
    return str(t)

# =============================
# 대진 생성 로직 (한울방식 포함)
# =============================
def make_kdk_hanul(players, target_games):
    n = len(players)
    key = str(target_games)
    if key in HANUL_KDK_DATA and n in HANUL_KDK_DATA[key]:
        # 랜덤 번호 부여
        shuffled_players = random.sample(players, n)
        match_strings = HANUL_KDK_DATA[key][n]
        
        # 경기당 최대 2코트(4명씩) 기준 혹은 이미지 순서대로 라운드 구성
        # 여기서는 이미지의 '순서'를 라운드로 취급하여 1경기당 1라운드로 배치
        rounds = []
        for ms in match_strings:
            t1_idx, t2_idx = ms.split(":")
            # 다중 숫자 처리 (예: 1112는 11번, 12번)
            def parse_indices(s):
                idxs = []
                temp = ""
                for char in s:
                    temp += char
                    if int(temp) <= n:
                        if len(temp) == 1 and char != '1': # 2~9번
                            idxs.append(int(temp)-1); temp = ""
                        elif len(temp) == 2: # 10~12번
                            idxs.append(int(temp)-1); temp = ""
                if temp: idxs.append(int(temp)-1)
                return idxs
            
            # 한울방식은 보통 한 팀에 2명씩. 12:34 형태
            # 복잡한 '910:1112' 같은 케이스를 위해 수동 매핑 로직 보강
            # 여기서는 단순화를 위해 글자 하나씩 자르되 10,11,12는 2글자로 인식
            def smart_split(s):
                res = []
                i = 0
                while i < len(s):
                    if s[i:i+2] in ['10', '11', '12']:
                        res.append(int(s[i:i+2])-1); i += 2
                    else:
                        res.append(int(s[i])-1); i += 1
                return res

            idx1 = smart_split(t1_idx)
            idx2 = smart_split(t2_idx)
            
            match = [[shuffled_players[i] for i in idx1], [shuffled_players[i] for i in idx2]]
            rounds.append([match])
        return rounds
    return None # 해당 데이터 없으면 일반 로직으로 전환

def make_round_robin(teams, target_games):
    matches = [[teams[i], teams[j]] for i in range(len(teams)) for j in range(i+1, len(teams))]
    random.shuffle(matches)
    rounds = []
    while matches:
        curr_round = []
        used = set()
        for m in matches[:]:
            p_in_m = set(m[0] + m[1])
            if not (p_in_m & used):
                curr_round.append(m); used.update(p_in_m); matches.remove(m)
        if not curr_round: break
        rounds.append(curr_round)
    return rounds

# =============================
# 매트릭스 생성 함수 (페어 표시 지원)
# =============================
def draw_match_matrix(group_name):
    players = st.session_state.groups[group_name]
    mode = st.session_state.modes[group_name]
    group_scores = {k: v for k, v in st.session_state.scores.items() if k[0] == group_name}
    
    # 헤더 결정 (고정페어면 팀명, 아니면 개인명)
    if mode == "고정페어":
        # 현재 그룹의 모든 페어 추출
        teams_list = []
        seen = set()
        for ri, rd in enumerate(st.session_state.schedule[group_name]):
            for mi, (t1, t2) in enumerate(rd):
                for t in [tuple(t1), tuple(t2)]:
                    if t not in seen:
                        teams_list.append(t); seen.add(t)
        entities = [f"({i+1}) " + " & ".join(t) for i, t in enumerate(teams_list)]
        entity_map = {t: name for t, name in zip(teams_list, entities)}
    else:
        entities = players
        entity_map = {tuple([p]): p for p in players}

    stats = {e: {"승":0, "패":0, "득실":0} for e in entities}
    matrix_data = {e: {other: "" for other in entities} for e in entities}

    for data in group_scores.values():
        t1, t2 = data["teams"]; s1, s2 = data["score"]
        
        # 맵핑을 위한 키 변환
        k1 = tuple(t1) if mode == "고정페어" else tuple([t1[0]])
        k2 = tuple(t2) if mode == "고정페어" else tuple([t2[0]])

        # 개인 통계 (항상 개인 단위로 집계)
        for team, my_s, op_s in [(t1, s1, s2), (t2, s2, s1)]:
            ent_key = entity_map.get(tuple(team)) if mode == "고정페어" else team[0]
            if ent_key in stats:
                if my_s > op_s: stats[ent_key]["승"] += 1
                elif my_s < op_s: stats[ent_key]["패"] += 1
                stats[ent_key]["득실"] += (my_s - op_s)
            
            # 매트릭스용
            o_key = entity_map.get(tuple(t2 if team==t1 else t1)) if mode == "고정페어" else (t2[0] if team==t1 else t1[0])
            if ent_key in matrix_data and o_key in matrix_data[ent_key]:
                matrix_data[ent_key][o_key] = f"{my_s}:{op_s}"

    st.subheader(f"📊 Group {group_name} 경기 현황 ({mode})")
    col1, col2 = st.columns([3, 2])
    with col1:
        st.markdown("**[상대별 전적 매트릭스]**")
        st.dataframe(pd.DataFrame.from_dict(matrix_data, orient='index'))
    with col2:
        st.markdown("**[순위 집계]**")
        summary_df = pd.DataFrame.from_dict(stats, orient='index').sort_values(["승", "득실"], ascending=False)
        st.dataframe(summary_df)

# =============================
# UI 섹션
# =============================
st.sidebar.title("🎾 두류 테니스")
menu = st.sidebar.radio("메뉴 이동", ["🏆 랭킹보드", "📅 대진표/입력", "📊 경기결과", "📜 지난 대회 기록", "⚙ 관리자 센터"])

if menu == "🏆 랭킹보드":
    st.markdown("<div class='main-title'>DU-RYU TENNIS RANKING</div>", unsafe_allow_html=True)
    df = load_rank()
    if not df.empty:
        df = df.sort_values("현재포인트", ascending=False).reset_index(drop=True)
        df.insert(0, "순위", range(1, len(df)+1))
        st.table(df[["순위", "이름", "현재포인트"]])
    else: st.info("데이터가 없습니다.")

elif menu == "📅 대진표/입력":
    st.markdown("<div class='main-title'>MATCH SCHEDULE</div>", unsafe_allow_html=True)
    if not st.session_state.schedule: st.warning("관리자 센터에서 대진을 생성하세요.")
    else:
        tabs = st.tabs([f"Group {g}" for g in st.session_state.schedule.keys()])
        for idx, g in enumerate(st.session_state.schedule.keys()):
            with tabs[idx]:
                draw_match_matrix(g)
                st.divider()
                p_list = st.session_state.groups[g]
                for ri, rd in enumerate(st.session_state.schedule[g]):
                    st.markdown(f"<h4 style='text-align:center; background:#f8f9fa;'>Round {ri+1}</h4>", unsafe_allow_html=True)
                    for mi, (t1, t2) in enumerate(rd):
                        c1, cv, c2 = st.columns([4, 1, 4])
                        score_key = (g, ri, mi)
                        existing = st.session_state.scores.get(score_key, {"score": (0, 0)})["score"]
                        with c1:
                            st.markdown(f"<div class='team-card match-bg-1'><div class='team-name'>{team_name(t1, p_list)}</div></div>", unsafe_allow_html=True)
                            s1 = st.number_input("점수", 0, 10, existing[0], key=f"s1_{g}_{ri}_{mi}", label_visibility="collapsed")
                        with cv: st.markdown("<div class='vs-text'>VS</div>", unsafe_allow_html=True)
                        with c2:
                            st.markdown(f"<div class='team-card match-bg-2'><div class='team-name'>{team_name(t2, p_list)}</div></div>", unsafe_allow_html=True)
                            s2 = st.number_input("점수", 0, 10, existing[1], key=f"s2_{g}_{ri}_{mi}", label_visibility="collapsed")
                        if st.button(f"저장 ({mi+1})", key=f"b_{g}_{ri}_{mi}"):
                            st.session_state.scores[score_key] = {"teams": (tuple(t1), tuple(t2)), "score": (s1, s2)}
                            st.rerun()

elif menu == "⚙ 관리자 센터":
    st.markdown("<div class='main-title'>ADMIN PANEL</div>", unsafe_allow_html=True)
    if st.text_input("비밀번호", type="password") == "0502":
        tab1, tab2 = st.tabs(["대회 생성", "데이터 관리"])
        with tab1:
            st.session_state.tournament_name = st.text_input("대회명", st.session_state.tournament_name)
            raw_p = st.text_area("참가자 (쉼표 구분)")
            g_count = st.number_input("그룹 수", 1, 5, 1)
            g_configs = {}
            for i in range(g_count):
                gn = chr(65+i)
                col = st.columns(3)
                with col[0]: sz = st.number_input(f"{gn} 인원", 2, 20, 8)
                with col[1]: md = st.selectbox(f"{gn} 방식", ["KDK", "고정페어", "단식"], key=f"m_{gn}")
                with col[2]: gc = st.selectbox(f"{gn} 경기수", [3, 4, 5, 6], index=1, key=f"c_{gn}")
                g_configs[gn] = (sz, md, gc)
            
            if st.button("대진 생성", type="primary"):
                p_list = [p.strip() for p in raw_p.split(",") if p.strip()]
                if len(p_list) < sum(c[0] for c in g_configs.values()):
                    st.error("인원이 부족합니다.")
                else:
                    # 랭킹순 정렬 로직 (생략 가능하나 유지)
                    rank_df = load_rank()
                    rank_map = dict(zip(rank_df['이름'], rank_df['현재포인트']))
                    p_list.sort(key=lambda x: rank_map.get(x, 0), reverse=True)
                    
                    st.session_state.groups = {}
                    curr = 0
                    for gn, (sz, md, gc) in g_configs.items():
                        group_p = p_list[curr:curr+sz]
                        st.session_state.groups[gn] = group_p
                        st.session_state.modes[gn] = md
                        
                        if md == "KDK":
                            hanul = make_kdk_hanul(group_p, gc)
                            if hanul: st.session_state.schedule[gn] = hanul
                            else: st.warning(f"{gn}조: 한울 데이터가 없어 자동생성으로 대체합니다."); # 기본 KDK 로직 연결 가능
                        elif md == "고정페어":
                            pairs = [group_p[i:i+2] for i in range(0, len(group_p), 2)]
                            st.session_state.schedule[gn] = make_round_robin(pairs, gc)
                        else:
                            singles = [[p] for p in group_p]
                            st.session_state.schedule[gn] = make_round_robin(singles, gc)
                        curr += sz
                    st.success("대진이 생성되었습니다.")
                    st.rerun()
